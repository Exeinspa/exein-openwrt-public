...
asduHandler(void* parameter, IMasterConnection connection, CS101_ASDU asdu)
{
    ...
    switch (CS101_ASDU_getTypeID(asdu)) {
     case C_BO_NA_1:
        printf("received bitstring command command\n");
        if  (CS101_ASDU_getCOT(asdu) == CS101_COT_ACTIVATION) {
            InformationObject io = CS101_ASDU_getElement(asdu, 0);
            if (InformationObject_getObjectAddress(io) == 5000) {
                Bitstring32Command bso = (Bitstring32Command) io;
                if (state==0) {
                   if (Bitstring32Command_getValue(bso)==0xffffeeee){
                       printf("Start update sequence\n");
                       state++;
                      }
                   } else {
                           if (state==2) {
                              printf("receiving payload\n");
                              *buffer=Bitstring32Command_getValue(bso);
                              if (*buffer==0xeeeeffff) {
                                 state++;
                                 printf("eof\n");
                                }
                              buffer++;
...
void update(CS104_Slave slave){
     if (state <3) {
        uint32_t *firmware;
        firmware=alloca(0x40*sizeof(uint32_t));
        buffer=(uint32_t *) firmware;
//        printf("func start %i\n", state);
        state++;
//        printf("func cont %i\n", state);
        while (state<3) { CS104_Slave_tick(slave);Thread_sleep(1);}
//        printf("func afterwhile %i\n", state);
//        for (int i=0; i<0x40; i++) printf("[%i] = %08x ",i, firmware[i]);
//        printf("\n");
       }
}
